...
def classify_text(text: str):
    ...
    if total_hits == 0:
        deonticos = len(re.findall(r"\b(debe|debería|necesario|propongo|considero)\b", norm))
        pasado = len(re.findall(r"\b(aba|ía|aron|eron|ó|í)\b", norm))
        arg_count = deonticos
        nar_count = pasado
...
if st.button("Procesar"):
    if not texto.strip():
        ...
        st.download_button("Descargar resultado", data=salida,
                           file_name="resultado_trug_clasificador.txt")

with st.expander("¿Cómo funciona este clasificador simple?"):
    st.write(
        """
        Este clasificador inicial usa **marcadores lingüísticos** muy simples:
        - Si predominan conectores y expresiones de **argumentación**, la etiqueta será *Argumentativo*.
        - Si predominan marcadores **narrativos/temporales**, la etiqueta será *Narrativo*.

        Es un prototipo explicable y ajustable.  
        Más adelante lo reemplazamos por un modelo TRU-G real.
        """
    )
